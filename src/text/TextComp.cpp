...
void TextComp::setLineSpacing(const float32_t spacingFactor)
{
    if (std::abs(m_infoText.getInfo<InfoText::Info::LINE_SPACING_FACTOR>() -
                 spacingFactor) > Constant::m_epsilon_f)
    {
...
{
    if (std::abs(m_widthWrap - widthWrap) > Constant::m_epsilon_f)
    {
...
sf::Vector2<float32_t> TextComp::findCharacterPos(std::size_t index) const
{
    ...
    if (nullptr != m_infoText.getInfo<InfoText::Info::FONT>())
    {
        ...
        // bool isBold =
        //     m_infoText.getInfo<InfoText::Info::STYLE>() &
        //     InfoText::Style::Bold;
        // float32_t whitespaceWidth =
        //     m_infoText.getInfo<InfoText::Info::FONT>()
        //         ->getGlyph(' ',
        //         m_infoText.getInfo<InfoText::Info::CHAR_SIZE>(),
        //                    isBold)
        //         .advance;
        ...
        //     m_infoText.getInfo<InfoText::Info::FONT>()->getLineSpacing(
        //         m_infoText.getInfo<InfoText::Info::CHAR_SIZE>()) *
        ...
        for (std::size_t i = 0; i < index; ++i)
        {
            ...
            while ((i - iInit) >= m_chainText[indexChain]->getNbChar(indexNode))
            {
                ...
                {
                    static_cast<void>(vecInfo.emplace_back(
            ...
            // {
            //     if (vecInfo[vecInfo.size() - 1 - j]->isUsed(
            //             InfoText::Info::STYLE))
            //     {
            //         curBold = vecInfo[vecInfo.size() - 1 - j]
            //                       ->getInfoText()
            //                       .getInfo<InfoText::Info::STYLE>() &
            //                   InfoText::Style::Bold;
            //         break;
            //     }
            // }
...
void TextComp::ensureGeometryUpdate() const
{
    ...
    // bool isBold =
    //     m_infoText.getInfo<InfoText::Info::STYLE>() & InfoText::Style::Bold;
    // bool isUnderlined = m_infoText.getInfo<InfoText::Info::STYLE>() &
    ...
    //         ? 0.209f
    //         : 0.f; // 12 degrees in radians float32_t underlineOffset    =
    // m_infoText.getInfo<InfoText::Info::FONT>()->getUnderlinePosition(
    //     m_infoText.getInfo<InfoText::Info::CHAR_SIZE>());
    // float32_t underlineThickness =
    //     m_infoText.getInfo<InfoText::Info::FONT>()->getUnderlineThickness(
    //         m_infoText.getInfo<InfoText::Info::CHAR_SIZE>());
    ...
    //     m_infoText.getInfo<InfoText::Info::FONT>()
    //         ->getGlyph('x', m_infoText.getInfo<InfoText::Info::CHAR_SIZE>(),
    //                    isBold)
    //         .bounds;
    ...
    // float32_t whitespaceWidth =
    //     m_infoText.getInfo<InfoText::Info::FONT>()
    //         ->getGlyph(' ', m_infoText.getInfo<InfoText::Info::CHAR_SIZE>(),
    //                    isBold)
    //         .advance;
    // float32_t letterSpacing =
    //     (whitespaceWidth / 3.f) *
         ...
         ++i)
    {
        ...
        if (changeInfo || (0 == i))
        {
                 ...
                 curCharSize))
            {
                ...
                static_cast<void>(m_vertices.emplace_back());
                static_cast<void>(m_outlineVertices.emplace_back());
        ...
        //     {
        //         TextComp::addLine(
        //             m_outlineVertices[indexArrayVert], x, y,
        //             curInfo.getInfoText()
    ...
    // curInfo.getInfoText().getInfo<InfoText::Info::FONT>()->getUnderlinePosition(
    //     curInfo.getInfoText().getInfo<InfoText::Info::CHAR_SIZE>());
